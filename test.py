...
X=None
Y=[]
for im in os.listdir('./PI_test'):
    photo=crop_center(np.array(Image.open("./PI_test/"+im)), 256, 256)
    red=photo[:, :, 0]
    green=photo[:, :, 1]
    blue=photo[:, :, 2]
...
for im in os.listdir('./CG_test'):
    photo=crop_center(np.array(Image.open("./CG_test/"+im)), 256, 256)
    red=photo[:, :, 0]
    green=photo[:, :, 1]
    blue=photo[:, :, 2]
    (A1r, (H1r, V1r, D1r))=pywt.dwt2(red, "haar")
...
###################################################################################################################
    ...
    if X is None:
        ...
        X=np.vstack((X, stat))
        Y.append(1)
classifier=joblib.load("data.pkl")
